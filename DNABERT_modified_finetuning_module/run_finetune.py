...
def train(args, train_dataset, model, tokenizer):
    ...
    # added by Piroozeh to freeze all model parameters except the classification layer
    if args.train_only_cls:
    ...
    # end of added by Piroozeh to freeze all model parameters except the classification layer
    ...
    # added by Piroozeh to print the number of trainable and frozen parameters
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f'Trainable parameters: {trainable_params}')
    frozen_params = sum(p.numel() for p in model.parameters() if p.requires_grad==False)
    print(f'Frozen parameters: {frozen_params}')
    # end of added by Piroozeh to print the number of trainable and frozen parameters
    # Train!
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        #added by Piroozeh to calculate train metrics
                        results_train = {}
                        ...
                        if args.do_ensemble_pred:
                            results_train = compute_metrics(args.task_name, preds, out_label_ids, probs[:,1])
                        else:
                            results_train = compute_metrics(args.task_name, preds, out_label_ids, probs)
                        results_train.update(results_train)
                        for key, value in results_train.items():
                            tr_key = "tr_{}".format(key)
                            logs[tr_key] = value
                        # end of added by Piroozeh to calculate train metrics
...
def evaluate(args, model, tokenizer, prefix="", evaluate=True):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        # added by Piroozeh to include evaluation loss in results
...
# Original script for visualizing attention scores. Can be used for small sized datasets,
#  which I used for my Rand_Neg and ACS_Neg dataset with train size 325 samples.
...
#         print(pred_dataset[0][0])
        
#         print("Decoded Texts:")
#         decoded_text = tokenizer.decode(pred_dataset[0][0], skip_special_tokens=False)
#         print(decoded_text)
...
# added by Piroozeh to modify visualization function to compute attention scores for large datasets.
def _get_last_layer_attention(outputs):
    """
    Returns last-layer attention tensor shaped (batch, heads, seq, seq).
    Works with Transformers-style outputs (tuple or ModelOutput).
    """
    if hasattr(outputs, "attentions") and outputs.attentions is not None:
        return outputs.attentions[-1]
    # Fallback to your original indexing pattern: outputs[-1] is attentions tuple
    return outputs[-1][-1]


def _compute_scores_cls_from_last_layer(
    last_layer_attn: torch.Tensor,
    kmer: int,
    counts: torch.Tensor,
) -> torch.Tensor:
    """
    last_layer_attn: (B, H, L, L)
    Returns: (B, L) scores, normalized like the original code.
    """
    b, h, l1, l2 = last_layer_attn.shape
    if l1 != l2:
        raise ValueError(f"Expected square attention, got {last_layer_attn.shape}")
    L = l1
    m = L - kmer + 1
    if m <= 0:
        raise ValueError(f"kmer={kmer} too large for seq_len={L}")

    # CLS row: attention from CLS token to all tokens (B, H, L)
    cls_row = last_layer_attn[:, :, 0, :]

    # Sum over heads => (B, L)
    cls_sum = cls_row.sum(dim=1)

    # Build a like original: i in [1, L-kmer+1] inclusive => length m
    a = cls_sum[:, 1 : 1 + m].clone()

    # Original rule:
    # for i in range(len(attn_score)-1):
    #   if attn_score[i+1] == 0: attn_score[i] = 0; break
    if m >= 2:
        zeros = (a[:, 1:] == 0)
        any_zero = zeros.any(dim=1)
        # argmax gives first 1 when any exists; else 0 (ignored by any_zero mask)
        first_zero_j = zeros.float().argmax(dim=1)  # j in [0..m-2]
        rows = torch.nonzero(any_zero, as_tuple=False).squeeze(1)
        if rows.numel() > 0:
            a[rows, first_zero_j[rows]] = 0  # set previous element to 0

    # Full convolution with ones(kmer): padding=kmer-1 yields output length m+kmer-1 = L
    kernel = torch.ones((1, 1, kmer), device=a.device, dtype=a.dtype)
    real_sum = F.conv1d(a.unsqueeze(1), kernel, padding=kmer - 1).squeeze(1)  # (B, L)

    real = real_sum / counts  # broadcast (L,)
    norm = torch.linalg.vector_norm(real, dim=1, keepdim=True)
    real = real / torch.clamp(norm, min=1e-12)
    return real

# modified by Piroozeh to compute attention scores for large datasets, which I used for K562 human origin datasets.
# Memory-optimized visualization path for large-scale attention extraction.
#
# Original implementation stored the full last-layer attention tensor for all
# samples: (N, H, L, L), which becomes prohibitively expensive for large
# datasets. This version computes the final CLS-based visualization scores
# directly on each batch and immediately discards the raw attention matrices.
#
# Additional optimizations:
# - Vectorized the k-mer score aggregation using conv1d instead of Python loops.
# - Computes scores on GPU and stores only the final (N, L) normalized scores.
# - Avoids materializing the full attention dataset in memory.
#
# Result: substantially lower memory usage and faster processing when
# extracting attention-derived visualization scores from large datasets.
def visualize(args, model, tokenizer, kmer, prefix=""):
    pred_task_names = (args.task_name,)
    pred_outputs_dirs = (args.predict_dir,)
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        num_examples = len(pred_dataset)
        num_classes = 3 if args.task_name == "dnasplice" else 2

        preds = np.zeros((num_examples, num_classes), dtype=np.float32)

        # Final output only: (N, L) instead of (N, H, L, L)
        L = args.max_seq_length
        scores = np.zeros((num_examples, L), dtype=np.float32)

        # Precompute counts once (length L)
        m = L - kmer + 1
        if m <= 0:
            raise ValueError(f"kmer={kmer} too large for max_seq_length={L}")

        # counts is deterministic triangle-ish vector, computed via conv of ones
        counts_np = np.convolve(np.ones(m, dtype=np.float32), np.ones(kmer, dtype=np.float32), mode="full")
        if counts_np.shape[0] != L:
            raise RuntimeError(f"counts length {counts_np.shape[0]} != L {L}")
        counts = torch.tensor(counts_np, device=args.device, dtype=torch.float32)
        ...
        for index, batch in enumerate(tqdm(pred_dataloader, desc="Predicting")):
            ...
            with torch.no_grad():
                ...
                # Force attention output robustly across Transformers versions
                # try:
                ...
                # else:
                #     # original: outputs[:2] -> (loss?), logits
                #     logits = outputs[1] if len(outputs) > 1 else outputs[0]

                # _, logits = outputs[:2]
                # preds[start:end, :] = logits.detach().cpu().numpy()
              

                last_attn = _get_last_layer_attention(outputs)  # (B, H, L, L)
                # Ensure L matches args.max_seq_length
                if last_attn.size(-1) != L:
                    raise ValueError(
                        f"Attention seq_len {last_attn.size(-1)} != args.max_seq_length {L}. "
                        f"Set args.max_seq_length to your actual sequence length."
                    )

                batch_scores = _compute_scores_cls_from_last_layer(
                    last_layer_attn=last_attn.float(),
                    kmer=kmer,
                    counts=counts,
                )
                scores[start:end, :] = batch_scores.detach().cpu().numpy().astype(np.float32, copy=False)
    ...
    return scores 
# end of modified by Piroozeh to compute attention scores for large datasets.
...
def main():
    ...
    parser.add_argument("--from_scratch", type=bool, required=False, help="Train model from scratch or by loading from a checkpoint.")
    parser.add_argument("--train_only_cls", type=bool, required=False, help="Train all parameters or only classification layer.")
    ...
    if not args.do_visualize and not args.do_ensemble_pred:
        ...
        # added by Piroozeh to have option to train model from scratch or use pre-trained model and finetune it.
    ...
    # end of added by Piroozeh to have option to train model from scratch or use pre-trained model and finetune it.
    ...
    if args.do_visualize and args.local_rank in [-1, 0]:
        ...
        for kmer in visualization_models:
            ...
            model.to(args.device)
            attention_scores = visualize(args, model, tokenizer, prefix=prefix, kmer=kmer)
